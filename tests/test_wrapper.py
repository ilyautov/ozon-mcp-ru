...
def test_entry_point_resolves_to_the_real_server():
    ...
    assert callable(ozon_mcp_ru.main), "точка входа не вызывается"
    assert ozon_mcp_ru.main.__module__.startswith("ozon_mcp")
...
def test_versions_agree():
    ...
    assert py == sj["version"] == sj["packages"][0]["version"] == ozon_mcp_ru.__version__
...
def test_readme_number_matches_the_catalog_that_ships():
    ...
    assert len(rows) == 441, "каталог изменился, пересобери обвязку генератором"
    assert f"**{len(rows)} " in readme
    assert counts == {"read": 190, "write": 240, "destructive": 11}
...
def test_skill_description_fits_the_spec():
    ...
    text = (ROOT / "skills" / "ozon-mcp" / "SKILL.md").read_text(encoding="utf-8")
    head = text.split("---")[1]
